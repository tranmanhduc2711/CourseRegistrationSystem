...
public class StudentDAO {
    ...
    public static List<StudentInCourse> getAllCourseOfStudent(String stuId) {
        session = HibernateUtil.getSessionFactory().openSession();
        List<StudentInCourse> attends = null;
        try {
            final String hql = "From StudentInCourse  where student_id = '" + stuId + "'";
            Query query = session.createQuery(hql);
            attends = query.getResultList();
        } catch (HibernateException e) {
            System.out.println(e);
        } finally {
            session.close();
